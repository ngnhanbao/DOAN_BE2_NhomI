...
<main class="pt-10 pb-20 max-w-7xl mx-auto px-6 lg:px-8 font-body">
    ...
    @if(session('success'))
        <div class="mb-6 flex items-center gap-3 px-5 py-4 bg-green-50 border border-green-200 text-green-700 rounded-xl text-sm font-bold shadow-sm">
    ...
    @if(session('error'))
        <div class="mb-6 flex items-center gap-3 px-5 py-4 bg-red-50 border border-red-200 text-red-700 rounded-xl text-sm font-bold shadow-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div class="lg:col-span-7 space-y-6">
            <div class="bg-surface-container-lowest rounded-xl overflow-hidden aspect-[4/5] flex items-center justify-center p-8 relative shadow-sm">
                <div class="absolute top-6 left-6 flex flex-col gap-2 z-10">
                    @if($product->is_new)
                        <span class="bg-primary text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-tighter">
                            New Arrival
                        </span>
                    @endif

                    @if($product->is_hot)
                        <span class="bg-tertiary-container text-on-tertiary-container text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-tighter">
                            Hot Sale
                        </span>
                    @endif
                </div>
            ...
            <div class="grid grid-cols-4 gap-4">
                @foreach($images as $img)
                    <div class="aspect-square bg-surface-container-lowest rounded-lg border {{ $img->is_primary ? 'border-2 border-primary' : 'border-outline-variant/30' }} overflow-hidden cursor-pointer hover:border-primary transition-all p-2 flex items-center justify-center group">
                        <img 
                            alt="Thumbnail"
                            class="w-full h-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform"
        ...
        <div 
            ...
            x-data="{
                selectedVariant: '{{ $firstVariant->variant_id ?? '' }}',
                selectedPrice: '{{ $firstVariant->price ?? $product->base_price }}'
            }"
        >
            <div>
                <h1 class="text-4xl font-black text-primary tracking-tight leading-tight mb-4 uppercase">
                    {{ $product->name }}
                </h1>
                ...
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <span class="text-on-surface-variant text-sm font-label">
                        (1,248 nhận xét)
                    </span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    @foreach($variants as $variant)
                        ...
                        <button 
                            ...
                            "
                            :class="selectedVariant == '{{ $variant->variant_id }}'
                                ? 'border-primary bg-primary/10 text-primary ring-2 ring-primary/20'
                                : 'border-outline-variant bg-surface-container-lowest text-on-surface'"
                            class="py-3 px-2 rounded-md border text-xs font-bold hover:border-primary hover:bg-primary/5 transition-all text-center"
                        >
                            {{ $attrs['RAM'] ?? '' }} {{ $attrs['ROM'] ?? '' }}
            ...
            {{-- MÀU SẮC --}}
            <div class="space-y-4">
                <label class="text-xs font-bold uppercase tracking-widest text-on-surface">
                    Màu sắc lựa chọn
                </label>
                ...
                <div class="flex flex-wrap gap-4">
                    <button 
                        type="button" 
                        class="group flex items-center gap-3 pr-4 pl-1 py-1 rounded-full border-2 border-primary bg-primary-fixed/20"
                    >
                    ...
                    <button 
                        type="button" 
                        class="group flex items-center gap-3 pr-4 pl-1 py-1 rounded-full border border-outline-variant"
                    >
            ...
            <div class="bg-primary-container p-4 rounded-xl text-white flex items-center gap-4 border border-primary-container/20 overflow-hidden relative">
                <div class="z-10 flex-1">
                    <p class="text-[10px] uppercase tracking-widest font-bold opacity-80 mb-1">
                        Ưu đãi độc quyền
                    </p>

                    <p class="text-sm font-medium">
                        Trả góp 0% qua thẻ tín dụng B-Tris Pay.
                    </p>
                </div>

                <span 
                    class="material-symbols-outlined text-4xl opacity-20 absolute -right-2 -bottom-2" 
            ...
            <div class="flex gap-4 items-center">
                <button 
                    type="button"
                    class="flex-1 bg-gradient-to-r from-[#003366] to-[#004a8f] text-white py-5 rounded-md font-bold text-sm tracking-widest uppercase hover:shadow-lg active:scale-[0.98] transition-all"
                >
                    MUA NGAY
                </button>
                ...
                @if($firstVariant)
                    <form action="{{ route('cart.add') }}" method="POST">
                        ...
                        <button 
                            type="submit"
                            class="p-5 rounded-md border-2 border-primary-container text-primary-container hover:bg-primary-container hover:text-white active:scale-95 transition-all group"
                        >
                ...
                @else
                    <button 
                        type="button"
                        disabled
                        class="p-5 rounded-md border-2 border-gray-300 text-gray-400 cursor-not-allowed"
                    >
    ...
    <div class="mt-24">
        <div class="flex items-center justify-between mb-10">
            <h2 class="text-2xl font-black text-primary tracking-tighter uppercase">
                Thông số kỹ thuật
            </h2>
    ...
    <div class="mt-24 bg-white rounded-2xl p-8 border border-slate-100 shadow-sm">
        <h2 class="text-2xl font-black text-primary tracking-tighter uppercase mb-6">
            Đặc điểm nổi bật
        </h2>
