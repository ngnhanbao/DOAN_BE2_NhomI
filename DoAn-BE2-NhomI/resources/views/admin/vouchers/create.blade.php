...
@section('content')
<div x-data="{ 
    code: '{{ old('code', 'TECHSPRING20') }}',
    type: '{{ old('type', 'percent') }}',
    value: '{{ old('value', 15) }}',
    min_order: '{{ old('min_order_value', 500000) }}',
    max_discount: '{{ old('max_discount', 100000) }}',
    start_at: '{{ old('start_at', '') }}',
    end_at: '{{ old('end_at', '2024-12-31') }}',
    is_active: {{ old('_token') ? (old('is_active') ? 'true' : 'false') : 'true' }}
}" class="pb-10">
                ...
                <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="usage_limit" value="{{ old('usage_limit') }}" placeholder="1000" class="w-full bg-[#F4F5F7] border-none rounded-xl py-3 px-4 text-sm font-bold text-[#0A2540] focus:ring-2 focus:ring-[#0A2540]/10">
                        </div>
                        <div>
